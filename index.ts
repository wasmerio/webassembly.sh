...
const uses = packageName == "sharrattj/bash" ? ["wasmer/neatvi", "cowsay"] : params.getAll("use");
...
async function main() {
  ...
  let sdkUrl = new URL("@wasmer/sdk", import.meta.url);
...
function connectStreams(instance: Instance, term: Terminal) {
  ...
  // Note we are doing a hack here to convert the EOLs to \r\n because
  // the terminal does not support convertEol.
  // Otherwise we would just do: `term.write(chunk)`.
  instance.stdout.pipeTo(
    new WritableStream({ write: chunk => term.write(decoder.decode(chunk).replace(/\n/g, "\r\n")) }),
  );
